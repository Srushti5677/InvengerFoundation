...
import {
  ...
  LogOut, Plus, CheckCircle, XCircle, TrendingUp, Heart, BookOpen
} from "lucide-react";
...
const AdminDashboard = () => {
  ...
  const [activeTab, setActiveTab] = useState<"campaigns" | "gallery" | "stories" | "projects" | "media">("campaigns");
  ...
  const loadData = async () => {
    try {
      const [cRes, gRes, sRes, pRes] = await Promise.all([
        fetch(`${API}/campaigns`),
        fetch(`${API}/gallery`),
        fetch(`${API}/stories`),
        fetch(`${API}/projects`)
        ...
        <div className="p-8">
              ...
              <div className="grid grid-cols-1 gap-3">
                {campaigns.map(c => (
                  <div key={c.id} className="bg-white p-4 rounded-2xl border border-slate-100 flex items-center gap-4 hover:shadow-md transition-all">
                    <img src={c.imageUrl || "https://via.placeholder.com/150"} className="w-16 h-16 rounded-xl object-cover" />
               ...
               <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                 {galleryItems.map(item => (
                   <div key={item.id} className="bg-white p-2 rounded-2xl border border-slate-100 group relative shadow-sm">
                     <img src={item.imageUrl} className="w-full h-40 rounded-xl object-cover" />
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-8 text-slate-800">
                 {stories.map(s => (
                   <div key={s.id} className="bg-white p-4 rounded-3xl border border-slate-100 flex gap-4 items-center">
                     <img src={s.imageUrl} className="w-24 h-24 rounded-2xl object-cover" />
                     <div className="flex-1">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                {projects.map(p => (
                  <div key={p.id} className="bg-white rounded-[40px] p-6 border border-slate-100 shadow-sm transition-all hover:shadow-2xl group flex flex-col">
                    <img src={p.mainImage} className="w-full h-64 object-cover rounded-[32px] mb-6 shadow-lg shadow-slate-200" />
          ...
          {activeTab === "media" && (
             <div className="bg-white p-10 rounded-[40px] border border-slate-100 text-center shadow-xl">
                ...
                <p className="text-slate-400 text-sm mt-2 mb-8">Upload files directly to Supabase storage to get URLs for campaigns or gallery.</p>
